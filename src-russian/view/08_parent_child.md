...
Приложение можно представить как вложенное дерево компонентов. Каждый компонент 
...
Представьте ситуацию: объявили компонент `<FancyButton/>`, добавляющий какие-то стили, логирование или что-то ещё к `<button/>`.
Хочется использовать `<FancyButton/>` в компоненте `<App/>`. Но как установить коммуникацию между ими двумя?
...
видите, что передаёте сигналы `WriteSignal` как этот по всему коду, вам следует всерьёз задуматься о том не
...
Rust замыкание `Fn(MouseEvent)` можно использовать напрямую вместо `Callback`:
...
можно добавить `on:` слушатель прямо в то место, этот компонент используется в макросе `view` в `<App/>`.
...
`<ButtonD/>` теперь уже не дочерний элемент `<App/>`, так что в его свойства нельзя просто передать `WriteSignal`.
...
Используя [`provide_context`](https://docs.rs/leptos/latest/leptos/fn.provide_context.html) и [`use_context`](https://docs.rs/leptos/latest/leptos/fn.use_context.html).
...
Можно установить прямую коммуникацию между  `<ButtonD/>` и `<App/>`. 
Фактически — и в этом состоит сила мелкозернистой реактивности — коммуникация идёт напрямую между нажатием на кнопку `<ButtonD/>`
и единственным текстовым узлом в `<App/>`. Это как если бы самих компонентов вовсе не существовало. И, ну... в среде выполнения они не существуют.
