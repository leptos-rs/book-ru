...
с полями ввода в Leptos. Те из вас, кто знаком с React, SolidJS или похожим фреймворком могут их знать:
...
после снятия фокуса с поля ввода. Обычно лучше подходит `on:input`, но мы хотим оставить вам свободу выбора.
...
Это то же само, но `node_ref` лучше поддерживается `rust-analyzer`)
...
Это может вызвать некорректный рендеринг `<textarea>` (и проблемы во время гидратации) если попытаться использовать этот элемент для
отображения какого-то динамического контента. 

Вместо этого можно передать нереактивное первоначальное значение внутрь `<textarea>` и использовать `prop:value`, чтобы задавать
