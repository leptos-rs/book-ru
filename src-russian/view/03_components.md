...
В Leptos, вы объявляете свойства добавляя дополнительные аргументы в функцию компонента.
...
Учтите, что нельзя объявлять необязательные обобщенные свойства. Давайте посмотрим, что будет если вы попробуете:
...
Компилятор может отобразить их с помощью сокращения, но вы не можете задать их.
