...
Если вы подкованы в вопросе, вы можете знать, что оба типа реализуют примесь `Fn() -> i32`. 
...
Это вполне оправданный способ написать данный компонент: `progress` теперь принимает любое значение реализующее примесь `Fn()`.
